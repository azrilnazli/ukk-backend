...
class ApprovalRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'email' => ['required', 'string', 'email', 'max:255',  \Illuminate\Validation\Rule::unique('companies')->ignore($this->user()->id) ],
            'phone' => ['required','regex:/^([0-9\s\-\+\(\)]*)$/', 'string', 'max:15'],
            'address' => ['required', 'string', 'max:500'],
            'postcode' => ['required', 'integer' ],
            'city' => ['required', 'string', 'max:100'],
            'states' => ['required', 'string', 'max:100'],
            'board_of_directors' => ['required', 'string', 'max:1000'],
